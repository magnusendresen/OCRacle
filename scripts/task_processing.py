...
async def get_exam_info() -> Exam:
    ...
    write_progress(
        [task_status[t] for t in range(1, total_task_count + 1)],
        1,
        {6: str(exam.total_tasks)},
    )
    write_progress(
        [task_status[t] for t in range(1, total_task_count + 1)],
        LLM_STEPS,
...
async def process_task(task_number: str, exam: Exam, llm_steps: int = LLM_STEPS) -> Exam:
